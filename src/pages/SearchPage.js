...
const SearchPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Try to fetch data from API first, then fallback to mock data
        let jobsData = [];
        let locationsData = [];
        let filtersData = { industries: [], jobTypes: [], experienceLevels: [] };

        try {
          // Fetch all required data with correct API prefix
          const [jobsResponse, locationsResponse, filtersResponse] = await Promise.all([
            axios.get('http://localhost:5000/api/jobs'),
            axios.get('http://localhost:5000/api/locations'),
            axios.get('http://localhost:5000/api/jobFilters')
          ]);

          jobsData = Array.isArray(jobsResponse.data) ? jobsResponse.data : [jobsResponse.data];
          locationsData = locationsResponse.data || [];
          filtersData = filtersResponse.data || { industries: [], jobTypes: [], experienceLevels: [] };
        } catch (apiError) {
          console.warn('API not available, using fallback data:', apiError);
          
          // Fallback data
          locationsData = [
            { id: 1, name: 'Hồ Chí Minh', jobCount: 845 },
            { id: 2, name: 'Hà Nội', jobCount: 723 },
            { id: 3, name: 'Đà Nẵng', jobCount: 241 },
            { id: 4, name: 'An Giang', jobCount: 38 },
            { id: 5, name: 'Bà Rịa - Vũng Tàu', jobCount: 79 },
            { id: 6, name: 'Bình Dương', jobCount: 156 },
            { id: 7, name: 'Cần Thơ', jobCount: 112 }
          ];
          ...
          filtersData = {
            ...
            jobTypes: [
              { id: 1, name: 'Full-time', count: 1200 },
              { id: 2, name: 'Part-time', count: 300 },
              { id: 3, name: 'Contract', count: 150 },
              { id: 4, name: 'Freelance', count: 200 },
              { id: 5, name: 'Internship', count: 100 }
            ],
            experienceLevels: [
              { id: 1, name: 'Fresher', count: 400 },
              { id: 2, name: 'Junior', count: 500 },
              { id: 3, name: 'Middle', count: 600 },
              { id: 4, name: 'Senior', count: 300 },
              { id: 5, name: 'Lead', count: 150 }
            ]
          };
        }
        ...
        const formattedIndustries = filtersData.industries.map(industry => ({
        ...
        const formattedJobTypes = filtersData.jobTypes.map(type => ({
        ...
        const formattedExpLevels = filtersData.experienceLevels.map(level => ({
  ...
  // Initialize filters from URL parameters after data is loaded
  useEffect(() => {
    if (locationOptions.length > 0 && industryOptions.length > 0) {
      // Initialize search filter from URL
      if (query) {
        setSelectedFilters(prev => ({ ...prev, search: query }));
      }
      
      // Initialize location filter from URL
      if (locationParam) {
        const locationIds = locationParam.split(',').filter(id => id.trim());
        setSelectedLocationValues(locationIds);
        
        // Get the location objects from the IDs
        const selectedLocs = locationOptions
          .filter(option => locationIds.includes(option.value))
          .map(option => option.label);
        
        setSelectedFilters(prev => ({ ...prev, location: selectedLocs }));
      }
      
      // Initialize category filter from URL
      if (categoryParam) {
        const categoryIds = categoryParam.split(',').filter(id => id.trim());
        setSelectedIndustryValues(categoryIds);
        
        // Get the industry objects from the IDs
        const selectedInds = industryOptions
          .filter(option => categoryIds.includes(option.value))
          .map(option => option.label);
        
        setSelectedFilters(prev => ({ ...prev, industries: selectedInds }));
      }
    }
  }, [query, locationParam, categoryParam, locationOptions, industryOptions]);
  ...
  const handleLocationChange = (values) => {
    ...
    const newSearchParams = new URLSearchParams(searchParams);
    if (values.length > 0) {
      newSearchParams.set('locations', values.join(','));
    } else {
            ...
            <Collapse defaultActiveKey={['1', '2', '3', '4']} bordered={false} expandIconPosition="end">
              <Panel header={<strong>Địa điểm</strong>} key="1">
                <Select
                  ...
                  tagRender={(props) => {
                    ...
                    const location = locationOptions.find(loc => loc.value === props.value);
                    const displayName = location ? location.label : props.label;
              ...
              <Panel header={<strong>Ngành nghề</strong>} key="2">
                <Select
                  ...
                  tagRender={(props) => {
                    // Find the industry name by ID to display only the name without job count
                    const industry = industryOptions.find(ind => ind.value === props.value);
                    const displayName = industry ? industry.label : props.label;
