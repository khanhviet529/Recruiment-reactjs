...
const MeetingsPage = () => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      Alert.success && Alert.success('Cuộc họp đã được xóa thành công!');
    } catch (error) {
      console.error('Error deleting meeting:', error);
      Alert.error && Alert.error('Có lỗi xảy ra khi xóa cuộc họp. Vui lòng thử lại.');
    } finally {
  ...
  const canDeleteMeeting = (meeting) => {
    ...
    // Can delete if meeting hasn't started or is already ended/cancelled
